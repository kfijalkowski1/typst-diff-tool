...
pub(crate) fn create_diff_ast_tree(file_path1: &String, file_path2: &String) -> Result<SyntaxNode> {
    ...
    let added_in1st_nodes: HashSet<SyntaxNode> = nodes1
        .iter()
        .filter(|&node| !nodes2.contains(node))
        .cloned()
        .collect();
    let added_in2nd_nodes: HashSet<SyntaxNode> = nodes2
        .iter()
        .filter(|&node| !nodes1.contains(node))
        .cloned()
        .collect();
    ...
    while i < len1 || j < len2 {
        if i < len1
            && j < len2
            && added_in1st_nodes.contains(&nodes1[i])
            && added_in2nd_nodes.contains(&nodes2[j])
        {
            if nodes1[i].kind() == nodes2[j].kind() {
                new_node =
                    find_difference_in_children(&nodes1[i], &nodes2[j], false, prev_node_kind);
                prev_node_kind = new_node.kind();
                nodes.push(new_node); // Modified case
            } else {
                let deleted_node =
                    add_color_to_every_block(&nodes1[i], NodeStatus::DELETED, prev_node_kind);
                let added_node =
                    add_color_to_every_block(&nodes2[j], NodeStatus::ADDED, prev_node_kind);
...
mod tests {
    ...
    fn test_add_bullet_point() {
        ...
        let path_to_new: String =
            "data/1_additional_bullet_point/added_bullet_point.typ".to_string();
        ...
        let expected_content: String =
            fs::read_to_string("data/1_additional_bullet_point/expected_added_bullet_point.typ")
    ...
    fn test_remove_bullet_point() {
        ...
        let expected_content: String =
            fs::read_to_string("data/2_delete_bullet_point/expected_removed_bullet_point.typ")
                .expect("Couldn't read file");
    ...
    fn test_add_and_remove_bullet_point() {
        let path_to_old: String =
            "data/2_5_add_and_delete_bullet_point/bullet_point.typ".to_string();
        let path_to_new: String =
            "data/2_5_add_and_delete_bullet_point/added_and_deleted_bullet_points.typ".to_string();
    ...
    fn test_modify_bullet_point() {
        ...
        let path_to_new: String =
            "data/3_modify_bullet_point/modified_bullet_point.typ".to_string();
        ...
        let expected_content: String =
            fs::read_to_string("data/3_modify_bullet_point/expected_modified_bullet_point.typ")
                .expect("Couldn't read file");
    ...
    fn test_bullet_point_switched_places() {
        ...
        let path_to_new: String =
            "data/4_bullet_points_switch_places/switched_bullet_point.typ".to_string();
    ...
    fn test_added_whole_paragraph() {
        ...
        let expected_content: String =
            fs::read_to_string("data/5_add_paragraphs/expected_added_paragraphs.typ")
                .expect("Couldn't read file");
