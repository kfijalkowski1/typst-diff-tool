...
use crate::node_handlers::helpers_with_syntax_kind::{
    is_function_argument_other_then_text, is_some_kind_of_whitespace, skip_syntax_kinds,
};

pub fn find_difference_in_children(
    child_old: &SyntaxNode,
    child_new: &SyntaxNode,
    is_an_argument_value: bool,
    previous_kind: SyntaxKind,
) -> SyntaxNode {
    let node_kind: SyntaxKind = child_new.kind();

    if is_some_kind_of_whitespace(&node_kind)
        || is_function_argument_other_then_text(node_kind, previous_kind)
    {
        child_new.clone()
    } else if child_old.children().count() == 0 && child_new.children().count() == 0 {
        if child_old.text() == child_new.text() {
            SyntaxNode::leaf(node_kind, child_new.text().to_string())
        } else {
            let combined_text: String =
                create_combined_text_diff(child_old, child_new, is_an_argument_value);
            ...
            loop {
                ...
                match (iter1.next(), iter2.next()) {
                    (Some(child1), Some(child2)) => {
                        if child1 != child2 {
                            combined_child = find_difference_in_children(
                                child1,
                                child2,
                    ...
                    (Some(child1), None) => {
                        combined_child =
                            add_color_to_every_block(child1, NodeStatus::DELETED, prev_node_kind);
                        leaves.push(combined_child);
                    }
                    (None, Some(child2)) => {
                        combined_child =
                            add_color_to_every_block(child2, NodeStatus::ADDED, prev_node_kind);
                        leaves.push(combined_child);
                    }
                    (None, None) => {
                        break;
